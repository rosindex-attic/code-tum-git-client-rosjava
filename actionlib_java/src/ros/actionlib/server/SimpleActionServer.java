...
public class SimpleActionServer<T_ACTION_FEEDBACK extends Message, T_ACTION_GOAL extends Message, T_ACTION_RESULT extends Message, T_FEEDBACK extends Message, T_GOAL extends Message, T_RESULT extends Message>
		implements
		ActionServerCallbacks<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT> {

	protected ActionServer<T_ACTION_FEEDBACK,
			T_ACTION_GOAL,
			T_ACTION_RESULT,
			T_FEEDBACK,
			T_GOAL,
	...
	protected SimpleActionServerCallbacks<T_ACTION_FEEDBACK,
			T_ACTION_GOAL,
			T_ACTION_RESULT,
			T_FEEDBACK,
			T_GOAL,
	...
	protected ServerGoalHandle<T_ACTION_FEEDBACK,
			T_ACTION_GOAL,
			T_ACTION_RESULT,
			T_FEEDBACK,
			T_GOAL,
			T_RESULT> currentGoal;
						...
						while (actionServer != null && actionServer.getNodeHandle().ok()) {
							...
							try {
								...
								if (isActive()) {
									...
									if (exception) {
										setAborted(actionServer.spec.createResultMessage(),
												"This goal was set to 'ABORTED' by the " +
												"simple action server due to an exception " +
												"in the user callback (blockingGoalCallback()).");
									} else if (isActive()) {
										...
										setAborted(actionServer.spec.createResultMessage(),
												"This goal was aborted by the simple " +
												"action server. The user should have set " +
												"a terminal status on this goal but did not.");
									}
