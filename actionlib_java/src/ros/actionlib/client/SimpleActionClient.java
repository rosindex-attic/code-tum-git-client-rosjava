...
import ros.actionlib.state.SimpleGoalState;
 ...
 * called.<br>
 * The SimpleActionClient provides convenience methods for waiting for the final
 * result and starting and stopping a spin thread that services the callbacks.
 * This spares the users the effort to call spin() or spinOnce() themselves.
 * 
...
public class SimpleActionClient<T_ACTION_FEEDBACK extends Message, T_ACTION_GOAL extends Message, T_ACTION_RESULT extends Message, T_FEEDBACK extends Message, T_GOAL extends Message, T_RESULT extends Message>
		implements ActionClientCallbacks<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT> {
	...
	/**
	 * Timer used to schedule spinOnce() calls from inside of the spin thread
	 */
	protected Timer spinTimer;

	/**
	 * Monitor used to synchronize access to the spin timer.
	 */
	private Object spinSync = new Object();

	/**
	 * Monitor used to wait for the result and get notified as soon as it
	 * arrives.
	 */
	 ...
	 * given name space on a given action specification. An extra thread that
	 ...
	 *            server)
	 * @param spec
	 *            The specification of the action the SimpleActionClient shall
	 *            use
	 * @param useSpinThread
	 *            A flag used to request a spin thread that services the
	 *            callbacks
	 * @throws RosException
	 *             If setting up the needed subscribers and publishers fail
	 */
	public SimpleActionClient(String nameSpace,
		...
		this.actionClient = new ActionClient<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT>(nameSpace, spec);
		this.callbacks = null;
	 ...
	 *             If setting up the needed subscribers and publishers fail
	 */
	public SimpleActionClient(NodeHandle parentNode, String nameSpace,
			ActionSpec<?, T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT> spec,
			boolean useSpinThread) throws RosException {

		this.actionClient = new ActionClient<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT>(parentNode, nameSpace, spec);
		this.callbacks = null;
		this.goalHandle = null;
		this.currSimpleState = new SimpleGoalState(SimpleGoalState.StateEnum.PENDING);
		if (useSpinThread) {
	 ...
	 *            A goal message
	 */
	public void sendGoal(T_GOAL goal) {
		sendGoal(goal, null);
	}

	/**
	 * Sends a goal message to the action server demanding callback methods
	 * which enable the user to track the progress of the goal. If there is
	 * another active goal currently being worked on by the server, tracking of
	 * that goal stops without canceling it.
	 * 
	 * @param goal
	 *            A goal message
	 * @param callbacks
	...
	/**
	 * Uses the {@linkplain #sendGoal(Message)} method to send out the given
	 * goal and then waits for the result using the
	 ...
	 *            A goal message
	 * @param execTimeout
	 *            The maximum duration that the SimpleActionClient is going to
	 *            wait for a result before canceling the goal. A zero length
	 *            duration results in unlimited waiting.
	 * @param preemptTimeout
	 *            The maximum duration that the SimpleActionClient is going to
	 *            wait for the action server's acknowledgment after canceling
	 *            the goal. A zero length duration results in unlimited waiting.
	 * @return The latest goal state
	 */
	public SimpleClientGoalState sendGoalAndWait(T_GOAL goal, Duration execTimeout, Duration preemptTimeout) {
	 ...
	 * certain circumstances enabling the user to track the progress of the
	 * goal.
	 * 
	 * @param goal
	 *            A goal message
	 ...
	 */
	public SimpleClientGoalState sendGoalAndWait(T_GOAL goal, Duration execTimeout, Duration preemptTimeout,
			SimpleActionClientCallbacks<T_FEEDBACK, T_RESULT> callbacks) {

		sendGoal(goal, callbacks);

		if (waitForResult(execTimeout)) {
			Ros.getInstance().logDebug("[SimpleActionClient] Goal was achieved within specified execTimeout [" + (execTimeout.totalNsecs() / 1000000) + " ms]");
		} else {

			cancelGoal();

			Ros.getInstance().logDebug("[SimpleActionClient] Goal wasn't achieved within specified execTimeout [" + (execTimeout.totalNsecs() / 1000000) + " ms]");
			if (waitForResult(preemptTimeout)) {
				Ros.getInstance().logDebug("[SimpleActionClient] Preempt finished within specified preemptTimeout [" + (preemptTimeout.totalNsecs() / 1000000) + " ms]");
			} else {
				Ros.getInstance().logDebug("[SimpleActionClient] Preempt didn't finish within specified preemptTimeout [" + (preemptTimeout.totalNsecs() / 1000000) + " ms]");
			}
	...
	public void cancelGoal() {
		...
		if (goalHandle == null || goalHandle.isExpired()) {
			Ros.getInstance().logError("[SimpleActionClient] Trying to cancelGoal() while no goal is active. You are incorrectly using SimpleActionClient!");
		} else {
			goalHandle.cancel();
		}
	...
	public void stopTrackingGoal() {
		...
		if (goalHandle == null || goalHandle.isExpired()) {
			Ros.getInstance().logError("[SimpleActionClient] Trying to stopTrackingGoal() while no goal is active. You are incorrectly using SimpleActionClient!");
		} else {
			goalHandle.shutdown();
		}
	 ...
	 * If the optional spin thread was used it will be stopped. 
	 * 
	...
	public T_RESULT getResult() {
		...
		if (goalHandle == null || goalHandle.isExpired()) {
			Ros.getInstance().logError("[SimpleActionClient] Trying to getResult() while no goal is active. You are incorrectly using SimpleActionClient!");
			return null;
		} else {
	...
	public SimpleClientGoalState getState() {
		...
		if (goalHandle == null || goalHandle.isExpired()) {
			Ros.getInstance().logError("[SimpleActionClient] Trying to getState() while no goal is active. You are incorrectly using SimpleActionClient!");
			return null;
		}
		...
		case DONE: {
			...
			case LOST:
				...
				break;
			default:
				Ros.getInstance().logError("[SimpleActionClient] Unknown TerminalState '" + terminalState + "'. This is a bug in SimpleActionClient!");
				break;
			}
			break;
		...
		case WAITING_FOR_CANCEL_ACK: {
			...
			case ACTIVE:
				...
				break;
			case DONE:
				Ros.getInstance().logError("[SimpleActionClient] In WAITING_FOR_RESULT or WAITING_FOR_CANCEL_ACK, yet we are in SimpleGoalState DONE. This is a bug in SimpleActionClient!");
				state.setState(SimpleClientGoalState.StateEnum.LOST);
				break;
			default:
				Ros.getInstance().logError("[SimpleActionClient] Unknown SimpleGoalState '" + currSimpleState + "'. This is a bug in SimpleActionClient!");
				break;
			}
			break;
		...
		default:
			Ros.getInstance().logError("[SimpleActionClient] Error trying to interpret CommState - " + commState);
			break;
		}
...
//	}
	...
	public boolean waitForResult(Duration timeout) {
		...
		if (timeout.isNegative()) {
			Ros.getInstance().logWarn("[SimpleActionClient] Timeouts can't be negative. Timeout is [" + (timeout.totalNsecs() / 1000000) + " ms]");
			return false;
		} else if (goalHandle == null || goalHandle.isExpired()) {
			Ros.getInstance().logError("[SimpleActionClient] Trying to waitForResult() while no goal is active. You are incorrectly using SimpleActionClient!");
			return false;
		}

		Time timeoutTime = Ros.getInstance().now().add(timeout);
		Duration loopCheckTime = new Duration(1, 0); // Check every second, if NodeHandle is ok

		synchronized (waitSync) {
			while (actionClient.getNodeHandle().ok()) {
	...
	@Override
	public void feedbackCallback(ClientGoalHandle<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT> goalHandle,
			T_FEEDBACK feedback) {

		if (this.goalHandle != goalHandle) {
			Ros.getInstance().logError("[SimpleActionClient] Got a callback on a goalHandle that the client is not tracking. This is an internal SimpleActionClient/ActionClient bug or a GoalID collision.");
		}
	...
	@Override
	public void transitionCallback(ClientGoalHandle<T_ACTION_FEEDBACK, T_ACTION_GOAL, T_ACTION_RESULT, T_FEEDBACK, T_GOAL, T_RESULT> goalHandle) {
		...
		switch (commState.getState()) {
		case WAITING_FOR_GOAL_ACK:
			Ros.getInstance().logError("[SimpleActionClient] BUG: Shouldn't ever get a transition callback for WAITING_FOR_GOAL_ACK");
			break;
		case PENDING:
			if (!currSimpleState.equals(SimpleGoalState.StateEnum.PENDING)) {
				Ros.getInstance().logError("[SimpleActionClient] Got a transition to CommState '" + commState + "' while client is in SimpleGoalState '" + currSimpleState + "'");
			}
			break;
		case ACTIVE:
			...
			case DONE:
				...
				break;
			default:
				Ros.getInstance().logFatal("[SimpleActionClient] Unknown SimpleGoalState '" + currSimpleState + "'");
				break;
			}
			break;
		...
		case DONE:
			...
			case ACTIVE:
				...
				break;
			case DONE:
				Ros.getInstance().logError("[SimpleActionClient] BUG: Got a second transition to 'DONE'");
				break;
			default:
				Ros.getInstance().logFatal("[SimpleActionClient] Unknown SimpleGoalState '" + currSimpleState + "'");
				break;
		...
		default:
			Ros.getInstance().logError("[SimpleActionClient] Unknown CommState '" + commState + "'");
			break;
		}
